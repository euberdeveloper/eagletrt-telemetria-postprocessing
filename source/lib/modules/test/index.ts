...
export function processLogsForTest(canLogPath: string | null, gpsLogPath: string | null, outputPath = 'result', throwError = false): void {
    ...
    if (canLogPath) {
        ...
        let messages = parseCanLog(text, false, throwError); text = '';
        messages.forEach(message => addMessage(tempResult, message)); messages = [];
    }
    ...
    if (gpsLogPath) {
        ...
        let messages = parseGpsLog(text, false, throwError); text = '';
        messages.forEach(message => addMessage(tempResult, message)); messages = [];
    }

    let result = tempResultToResult(tempResult); tempResult = {};
